...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    struct bme280_data save_Data[12];
    int i=0;
    FILE *fp;
    fp=fopen("exer2.csv","a");
    fprintf(fp,"Temperature, Pressure, Humidity, Date and Time\n");
    fclose(fp);
    while (1)
    {
        ...
        if(i==10){
            saveDataCsv(&save_Data[0]);
            i=0;
        }
        save_Data[i] = comp_data;
        i++;
...
void saveDataCsv(struct bme280_data *comp_data)
{
    ...
    for(int i=0;i<10;i++){
        ...
        hum += comp_data->humidity;
        comp_data++;
        //printf("i = %d %0.2lf deg C, %0.2lf hPa, %0.2lf%% \n",i,temp, press, hum);
    }
    ...
    fprintf(fp,"%0.2lf deg C, %0.2lf hPa, %0.2lf%%, %s", temp/10, press/10, hum/10, asctime(tm));
    printf("%0.2lf deg C, %0.2lf hPa, %0.2lf%%, %s", temp/10, press/10, hum/10, asctime(tm));
    fclose(fp);
}
